...
public class FileParser {
	...
	public FileParser(String content) {
		...
		ASTCreator creator = new ASTCreator(content);
...
//    }
	...
	public HashMap<String, ArrayList<String>> getMethodStringList(){
		HashMap<String, ArrayList<String>> methodStrings = getMethodStrings();;
		return methodStrings;
	}
	
	public HashMap<String, ArrayList<String>> getMethodStrings() {
		HashMap<String, ArrayList<String>> methodStrings = new HashMap<String, ArrayList<String>>();
		for (int i = 0; i < compilationUnit.types().size(); i++) {
			if (compilationUnit.types().get(i) instanceof TypeDeclaration) {
				...
				for (int j = 0; j < k; j++) {
					MethodDeclaration methodDecl = methodDeclaration[j];					
//					String callType = "";
//					if(methodDecl.modifiers().size()>0)
//						callType = methodDecl.modifiers().get(0).toString()+" ";
					String methodName = methodDecl.getName().getFullyQualifiedName();
					...
					for (int l = 0; l < methodDecl.parameters().size(); l++) {
						parameters += ((SingleVariableDeclaration) methodDecl.parameters().get(l)).getType().toString();
//						parameters += " "+((SingleVariableDeclaration) methodDecl.parameters().get(l)).getName().toString();
						if(l != methodDecl.parameters().size()-1)
							parameters += ", ";
					}
					parameters = parameters.trim();
					methodName = returnTypeString+" \n"+methodName+" \n"+parameters+" \n";
					
					ArrayList<String> texts = new ArrayList<String>();
					texts.add(methodDecl.toString());
					if(methodDecl.getJavadoc() != null)
						texts.add(methodDecl.getJavadoc().toString());
					methodStrings.put(methodName, texts);
				}
			}
		}
		return methodStrings;
			...
			for (int i = 0; i < compilationUnit.types().size(); i++) {
				if (compilationUnit.types().get(i) instanceof TypeDeclaration) {
					...
					for (int j = 0; j < k; j++) {
						...
						// debug code
//						System.out.println(methodDecl+" "+methodDecl.getJavadoc());
//						System.out.printf(">>> Method: %s %s(%s)\n", returnTypeString, methodName, parameters);
						Method method = new Method(methodName, returnTypeString, parameters);
						allMethodList.add(method);
